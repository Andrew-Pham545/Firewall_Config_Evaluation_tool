...
def select_profile():
    ...
    if 0 <= choice_of_profile < len(profile_names):
        ...
        print('  3. Read data from template')
        print(Fore.YELLOW + "- REPORT Generation")
        ...
        if action == '1':
            ...
            if template_exists:
                ...
                print(Fore.GREEN + f"Read succesfully from the sheet and saved to: {profile_path}")
            else:
                print(Fore.RED + "Read failed, Template file not found in the profile directory.")
        elif action == '4':
            print(Fore.YELLOW + "\nDo you want to include AI evaluation? (y/n)")
            ai_choice = input(Fore.WHITE + "Choose an option (y or n): ").strip().lower()
            if ai_choice == 'y':
